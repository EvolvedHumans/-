...
public class SerialPortUtils extends Activity{
    ...
    /**
     * 在Main中判断SerialPort中Catch情况,只需要判断它是否崩溃。
     * 崩溃则openSerialPort方法
     */
    /**
     * 打开串口
     ...
     */
    public SerialPort openSerialPort(String path,int baudrate,int flag){
        try {
                serialPort = new SerialPort(new File(path),baudrate,0);

                //第一次启动，若无法连接，无限等待，第一次必须要实例化
                //如果实例化过程中出现崩溃现象，则通过catch返回null重连，从而达到为重连
    ...
    public void sendSerialPort(String data) throws InterruptedException {
        try {
            byte[] sendData = data.getBytes("Utf8"); //string转byte[]
        ...
        Log.e("阻塞时间",String.valueOf(d)+"ms");
...
//
//                        Log.i("读取到终止位时的全部数据",new String(buffer));
...
//                        Log.i("去掉终止位时的全部数据",new String(byte_buffer));
                        break; //运行到这，跳出循环
                    }
    ...
    public static interface OnDataReceiveListener {
        public void onDataReceive(byte[] buffer,int factPackLen,int cursor,int headerLength) throws UnsupportedEncodingException;
    }
